...
type Product = { id: string; description: string; unitPrice: number; packetsPerBox: number | null };
...
export default function NewInvoicePage() {
  ...
  const [lines, setLines] = useState<LineRow[]>([
    { productId: "", description: "", qty: 1, unitPrice: 0, lineTotal: 0 },
  ]);
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch("/api/users").then((r) => {
        if (!r.ok) {
          setUsersError("Could not load salespeople.");
          return [];
        }
        return r.json();
      ...
      if (me?.id && Array.isArray(u) && u.length > 0) {
        const current = u.find((x: User) => x.id === me.id);
        if (current) setSalespersonId(current.id);
        else if (u.length === 1) setSalespersonId(u[0].id);
      }
      setFetching(false);
    });
  }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!customerId || !salespersonId) {
      setError("Select customer and salesperson.");
      return;
    }
    setLoading(true);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              ...
              aria-describedby={users.length === 0 ? "salesperson-help" : undefined}
            >
              <option value="">Select…</option>
              {users.map((u) => (
            ...
            {users.length === 0 && (
              <p id="salesperson-help" className="mt-2 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded p-2">
                No salespeople in the list. Salespeople are <strong>registered staff</strong>.{" "}
                <Link href="/register" className="text-blue-600 hover:underline font-medium">
                  Register a new user
                </Link>{" "}
                (e.g. with a Salesperson code like &quot;AD&quot;) and they will appear here. You can also log out and register another account yourself.
              </p>
            )}
